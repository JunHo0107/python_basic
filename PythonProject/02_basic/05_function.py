...
ageA = 38
# 중복코드는 나중에 수정할 때 전부 고쳐야 되므로 귀찮아진다.
...
# 20부터 40까지 다 더한 값을 출력
sum = 0
for i in range(20, 41):
    sum += i
print(sum)

# 35부터 45까지 다 더한 값을 출력
sum = 0
for i in range(35, 46):
    sum += i
print(sum)

# 46부터 51까지 다 더한 값을 출력
sum = 0
...
# numArray의 평균값 계산 후 출력

# 배열 안에 있는 값들을 모두 다 더하고 5로 나누기
sum = 0
for i in numArray:
    sum += i
mean = sum / len(numArray) # len(numArray) 자리에 숫자 작성 = 하드코딩
print(mean)
...
for i in intArray:
    sum += i
    mean = sum / len(intArray)  # len(intarray) 자리에 숫자 작성 = 하드코딩
    print(mean)
